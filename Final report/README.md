...
### **改進之處**
* 策略面：
1. 上述指標找出之個股在初期出現時可能為強勢股，但到了後期這些股變得相對弱勢後，可能還是會被這套算法找出來(因為累加值須隨時間而遞減)，因此未來可能會多一欄資訊說明個股已出現在清單上的累積天數。
2. 此策略並沒有透過回測進行績效驗證，目前觀察可能有追高的情形，下圖為例：
!img[例子](https://i.imgur.com/IQ5vjkr.png)
* 程式面：
1. 因為是透過Windows Task Scheduler自動運行，若運行時處於沒網路的狀態則無法運行，因此未來期望能放在雲端自動運行。
2. 若發生程式無法運行的問題代表當日籌碼資料未被更新，目前此程式沒有設置自動檢查機制，若中間有缺資料未來計算之指標將失真，因此應設置機制讓其運行時先檢查前幾日之資料是否存在，若否，則先補齊原先資料後繼續運行。
...
5. "2018_outstanding_stock.csv" 則儲存2018/5/15日獲取之淨流通在外張數(流通在外張數-董監持股張數)，將會依據此csv檔中涵蓋的上市櫃公司計算指標，若此公司在此日期之前下市，則資料可能含有該個股，但由於未在此csv檔案中，所以不會被計算指標，此資料由TEJ資料庫下載後整理而得。
6. 證交所與櫃台買賣中心資料公布時間不一定，有時下午4, 5點便公布，有時則晚上9, 10點。
7. 太頻繁爬取可能會被擋，需爬取多筆歷史資料時建議使用time.sleep()拉長間隔時間。
8. 若遇到假日此程式會說明"今日休市!"的資訊。
